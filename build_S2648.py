...
for i in range(len(data)):
    ...
    try:
        ...
        sr_curves_inv = np.load(filename_inv+'_SR_curves.npy', allow_pickle=True)

        #sr_rates = np.load(filename+'_SR_rates.npy', allow_pickle=True)
        #sr_rates_inv = np.load(filename_inv+'_SR_rates.npy', allow_pickle=True)

        sr_fcurves = np.load(filename+'_SR_fcurves.npy', allow_pickle=True)
        sr_fcurves_inv = np.load(filename_inv+'_SR_fcurves.npy', allow_pickle=True)

        print(np.shape(sr_fcurves))
        print(np.shape(sr_fcurves_inv))

        #sr_frates = np.load(filename+'_SR_frates.npy', allow_pickle=True)
        #sr_frates_inv = np.load(filename_inv+'_SR_frates.npy', allow_pickle=True)

        #print(np.shape(sr_frates))
        #print(np.shape(sr_frates_inv))
        ...
        feat_srcurves.append(sr_curves)
        feat_srcurves.append(sr_curves_inv)
        #feat_srrates.append(sr_rates)
        #feat_srrates.append(sr_rates_inv)
        feat_srfcurves.append(sr_fcurves)
        feat_srfcurves.append(sr_fcurves_inv)
        #feat_srfrates.append(sr_frates)
        #feat_srfrates.append(sr_frates_inv)
...
feat_srcurves = np.array(feat_srcurves)
feat_srfcurves = np.array(feat_srfcurves)
#feat_srrates = np.array(feat_srrates)
#feat_srfrates = np.array(feat_srfrates)
y_ = np.array(y_)
y_ = np.round(y_, 3)
...
print(np.shape(feat_srcurves))
print(np.shape(feat_srfcurves))
#print(np.shape(feat_srrates))
#print(np.shape(feat_srfrates))
print(np.shape(y_))
...
np.save(f'X_{args.data}_SRcurves.npy', feat_srcurves)
#np.save(f'X_{args.data}_SRrates.npy', feat_srrates)
np.save(f'X_{args.data}_SRfcurves.npy', feat_srfcurves)
#np.save(f'X_{args.data}_SRfrates.npy', feat_srfrates)
